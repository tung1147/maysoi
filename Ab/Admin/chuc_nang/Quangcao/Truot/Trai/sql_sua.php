<?php
	...
	if($anh_hop_le == "Yes")
	{
		...
		if($row == 0 or $image_trung =="Yes")
		{
			...
			#echo $image ;
			mysql_query("UPDATE quang_cao SET link = '$_POST[txtLien_ket]',ten_file ='$image'  WHERE quang_cao.id =$_GET[id] LIMIT 1");
			
		}else
			{
					thongbao("Tên hình dã có.\\nCh?n tên khác");
			}
			}else
			{
			thongbao("Ðuôi file ?nh không h?p l?.\\n File h?p l?: $file_hop_le");
			}	
