...
function phan_trang($st)
	...
	function chuoi_idmenu_con($id_cha,$chuoi)
	{
		...
		while($fetch=mysql_fetch_array($query))
		{
			...
			if($r_2[0]!=0)
			{
				$chuoi=chuoi_idmenu_con($fetch['id'],$chuoi);
			}
		}
		return $chuoi;
	}
	
	function mang_idmenu_con($id_menu)
	{
		if($id_menu!="")
		{
			$chuoi=$id_menu.",";
		...
		$chuoi_idmenu_con=chuoi_idmenu_con($id_menu,$chuoi);
		$mang=explode(",",$chuoi_idmenu_con);
...
<?php
	function display($td,$link_hinh,$gia,$link_chi_tiet,$id_sp)
	{
	 	?>
			<td>
				<div>
...
<div class="display_td">
	<b><?php echo $tieude_menu; ?></b>
</div>
<div class="display_nd">
	<center>
		<table class="san_pham" id="dan_san_pham_abc">
			<?php
				$tma="image/San_pham/";
				$r=1;
				while($fetch=mysql_fetch_array($query))
				{
					...
					display($fetch['ten'],$link_hinh,$gia_1,$link_chi_tiet,$id_sp);
					if($r==$so_dl_1_hang)
					{
						$r=1;
						echo "</tr>";
